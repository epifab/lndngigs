...
from lndngigs.factories import get_logger, get_slack_bot, get_redis_client
...
def run(location, events_date, channel):
    ...
    bot = get_slack_bot(logger=logger, config=config, redis_client=get_redis_client(config))
